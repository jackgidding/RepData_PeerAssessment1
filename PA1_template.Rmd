...
There are `r format(round(nrow(data.activity),0),big.mark=",")` obvservations in the data set. 
...
There are `r format(round(nrow(data.activity.clean),0),big.mark=",")` observations in the clean dataset. A total of `r format(round(NROW(data.activity[is.na(data.activity[,1]),]),0),big.mark=",")` observations with NA values were dropped from the original dataset.
...
```
The mean number of steps per day is `r format(round(analysis.meanSteps,0),big.mark=",")` and the median number of steps per day is `r format(round(analysis.medianSteps,0),big.mark=",")`
...
Then period with the max number of steps is `r formattedInterval`. This is consistent with the data displayed in the time series chart above.
...
There are `r format(round(NROW(data.activity[is.na(data.activity[,1]),]),0),big.mark=",")` observations with missing values in the initial dataset. 
```{r}
...
To confirm, the new dataset now has `r format(round(NROW(data.activity[is.na(data.complete[,1]),]),0),big.mark=",")` observations with missing values. 
...
```
The mean number of steps per day is `r format(round(analysis.meanSteps,0),big.mark=",")` and the median number of steps per day is `r format(round(analysis.medianSteps,0),big.mark=",")`. The mean is the same as the cleaned dataset and the median has increased by 1. This is not significantly different. Thus the characteristics of the cleaned dataset (above) have been preserved when the NA values were replaced with the mean number of steps per day. The missing values therefore do not have a big impact on the analysis.
...
The red line on the charts shows the mean number of steps per 5-minute interval across all days and time intervals. The mean is `r format(round(mean(data.activity.clean$steps),0))` steps. 
